...
function PieChartCard({
  ...
  return (
    <article className='rounded-xl border border-slate-200 bg-gradient-to-br from-white to-slate-50 p-5 shadow-sm dark:border-slate-800 dark:from-slate-900 dark:to-slate-950'>
      <h3 className='text-sm font-semibold text-slate-700 dark:text-slate-200'>{title}</h3>
      <div className='mt-4 flex flex-col items-center gap-4 md:flex-row'>
        <div className='relative'>
          <svg width='180' height='180' viewBox='0 0 180 180' className='-rotate-90'>
            <circle cx='90' cy='90' r={radius} fill='transparent' stroke='#e2e8f0' strokeWidth={strokeWidth} />
            {total > 0 &&
              data.map((item) => {
  ...
  return (
    <AppLayout>
        ...
        <PageHeader
          ...
          action={
            <div className='flex flex-wrap items-end gap-2'>
              <div>
                <label htmlFor='startDate' className='mb-1 block text-xs font-semibold text-slate-600 dark:text-slate-300'>
                  Start date
                </label>
                <input
                  id='startDate'
                  ...
                  className='rounded-lg border border-blue-200 bg-white px-3 py-2 text-sm outline-none focus:border-blue-500 dark:border-slate-700 dark:bg-slate-900'
                />
              </div>
              <div>
                <label htmlFor='endDate' className='mb-1 block text-xs font-semibold text-slate-600 dark:text-slate-300'>
                  End date
                </label>
                <input
                  id='endDate'
              ...
              <button
                ...
                className='inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-blue-600 to-indigo-600 px-4 py-2 text-sm font-medium text-white shadow hover:from-blue-700 hover:to-indigo-700'
              >
      ...
      <section className='mt-4 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4'>
        <StatCard title='New Tickets' value={ticketStats.newTickets} icon={FiClipboard} className='border-blue-200 bg-blue-50/60 dark:border-blue-900/60 dark:bg-blue-900/10' />
        <StatCard title='In Progress Tickets' value={ticketStats.inProgress} icon={FiTool} className='border-amber-200 bg-amber-50/60 dark:border-amber-900/60 dark:bg-amber-900/10' />
        <StatCard title='Completed Tickets' value={ticketStats.completed} icon={FiCheckCircle} className='border-emerald-200 bg-emerald-50/60 dark:border-emerald-900/60 dark:bg-emerald-900/10' />
        <StatCard title='Cancelled Tickets' value={ticketStats.cancelled} icon={FiXCircle} className='border-rose-200 bg-rose-50/60 dark:border-rose-900/60 dark:bg-rose-900/10' />
      </section>

      <section className='mt-4 grid grid-cols-1 gap-4 md:grid-cols-3'>
        <StatCard title='Available Devices' value={deviceStats.available} icon={FiHardDrive} className='border-teal-200 bg-teal-50/60 dark:border-teal-900/60 dark:bg-teal-900/10' />
        <StatCard title='In Maintenance Devices' value={deviceStats.inMaintenance} icon={FiTool} className='border-orange-200 bg-orange-50/60 dark:border-orange-900/60 dark:bg-orange-900/10' />
        <StatCard title='Broken Devices' value={deviceStats.broken} icon={FiAlertTriangle} className='border-red-200 bg-red-50/60 dark:border-red-900/60 dark:bg-red-900/10' />
      </section>
      ...
      <section className='mt-4 grid grid-cols-1 gap-4 xl:grid-cols-3'>
          ...
          <DataTableWrapper title='Top Defective Devices'>
            <div className='overflow-x-auto'>
              <table className='min-w-full text-left text-sm'>
                ...
                <tbody>
                  {loading ? (
                    <tr><td className='px-3 py-4 text-slate-500 dark:text-slate-400' colSpan={2}>Loading data...</td></tr>
                  ) : (data?.topDefectiveDevices?.length ?? 0) === 0 ? (
                    <tr><td className='px-3 py-4 text-slate-500 dark:text-slate-400' colSpan={2}>No defective device data in this period.</td></tr>
                  ) : (
        ...
        <StatCard
          ...
          description='From assignment to completion confirmation'
          icon={FiClock}
